...
class PhoneLocation:
    ...
    @staticmethod
    def _load_data(data_file: str) -> Dict[str, Dict[str, str]]:
    ...
    def extract_area_code(self, phone_digits: str) -> Optional[str]:
    ...
    def query(self, phone: str) -> Optional[Dict[str, str]]:
        ...
        if not digits:
            return None
        area_code = self.extract_area_code(digits)
        if not area_code:
            return None
        result = self._area_codes.get(area_code)
        if result:
            return {
                "area_code": area_code,
